...
if ($payload && $payload['aud'] == $google_oauth_client_id) {
    ...
    if ($stmt = $con->prepare('SELECT id, accounttype, username, picture FROM accounts WHERE email = ?')) {
        ...
        if ($stmt->num_rows > 0) {
            $stmt->bind_result($id, $accounttype, $setusername, $existing_picture);
            $stmt->fetch();
            ...
            if ($stmt = $con->prepare('INSERT INTO accounts (fullname, password, email, ref, refcode, picture) VALUES (?, ?, ?, ?, ?, ?)')) {
                ...
                $remember_token = bin2hex(random_bytes(32));
                if ($stmt = $con->prepare('UPDATE accounts SET remember_token = ? WHERE email = ?')) {
                    $stmt->bind_param('ss', $remember_token, $email);
                    $stmt->execute();
                    setcookie('remember_token', $remember_token, time() + 31536000, '/');
                }
                ...
                $email->setSubject("New user for Voice " . date("Y-m-d"));
                $email->addTo("easyaistudio@gmail.com", "EasyAi Studio");
                $email->addContent("text/plain", $useremail . " just signed up for Voice, time to start coding.");
                $email->addContent(
                    "text/html",
                    "<strong>" . $useremail . " just signed up for Voice, time to start coding.</strong>"
                );
